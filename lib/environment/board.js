...
class Board {
  constructor ({ width, height, boulderCount, boulders, walls }) {
    this._width = width
    this._height = height
    this._boulders = []
    this._walls = []
  ...
  createWalls (wallCoords) {
    _.each(wallCoords, (coord) => {
      if (!this._isCoordinateAvailable(coord)) { return }

      const wall = new Tree(coord)
  ...
  getModelFromPosition (coord) {
    return _.find(this.allModels(), (model) => {
  ...
  _occupiedCoordinates () {
    return (
      _.map(this.allModels(), (model) => { return model.coordinates() })
    )
  }
