...
class Karel {
  ...
  turnLeft () {
    this._turn(TURN_ANGLE)
  }

  turnRight () {
    this._turn(-1 * TURN_ANGLE)
  }
  ...
  _turn (angle) {
    let turnedAngle = DIRECTION_ANGLES[this._direction] + angle
    turnedAngle = this._mod(turnedAngle, FULL_CIRCLE_ANGLE)

    this._direction = this._keyByValue(DIRECTION_ANGLES, turnedAngle)
  }
